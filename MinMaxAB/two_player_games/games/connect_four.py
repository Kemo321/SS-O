...
class ConnectFour(Game):
    ...
    def get_score(self, heuristic: Callable) -> float:
        winner = self.get_winner()
        """
        Returns:
            Score of the game state for the current player
        """
        if winner == self.get_current_player():
            return float('inf')
...
class ConnectFourState(State):
    ...
    def undo_move(self, move: ConnectFourMove) -> 'ConnectFourState':
        new_fields = [list(column) for column in self.fields]
        for i, field in enumerate(reversed(new_fields[move.column])):
            if field is not None:
                new_fields[move.column][len(new_fields[move.column]) - i - 1] = None
                break
        return ConnectFourState(current_player=self._other_player, other_player=self._current_player, fields=new_fields)
